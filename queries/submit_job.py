...
def submit_job(fname_sql,output_format,do_preview=False) :
    command="python hscReleaseQuery.py --user=nsevilla@local"
    if do_preview :
        command+=" -p"
...
def write_boxsearch(tablename,dec_range,ra_range,fname_out,output_format="fits",do_box=True,submit=False) :
    ...
    def add_filters(name,behind=True) :
        ...
        for fi in filters :
            if behind :
                ...
                sthere+="       ,"+fi+name+"\n"
        return sthere
    def add_filters_conditions(name,behind=False) :
        sthere=""
        for fi in filters :
            if behind :
                sthere+="       and not "+name+fi+"\n"
            else :
                sthere+="       and not "+fi+name+"\n"
        return sthere
    ...
    #Classification
    stout+=add_filters("classification_extendedness")
    #Dust extinction
    ...
    stout+="       "+tablename+"\n"
    stout+="WHERE\n"
    stout+="       detect_is_primary\n"
    stout+=add_filters_conditions("centroid_sdss_flags")
    stout+=add_filters_conditions("flags_pixel_interpolated_any")
    stout+=add_filters_conditions("flags_pixel_saturated_any")
    stout+=add_filters_conditions("flux_psf_flags")
    stout+=add_filters_conditions("cmodel_flux_flags")
...
def write_fieldsearch(tablename,fieldname,fname_out,output_format="fits",do_field=True,submit=False,ra_range=None,do_photoz=False,is_random=False) :
    filters=['g','r','i','z','y']
    ...
    if not is_random:
        ...
        #stout+="       ,iclassification_extendedness\n"
        #Classification
        stout+=add_filters("classification_extendedness",behind=False)
        #Dust extinction
        stout+=add_filters("a_")
        #Psf fluxes and magnitudes
        #stout+=add_filters("flux_psf",behind=False)
    ...
    #stout+="       iflags_pixel_cr_center=False and\n"
    #stout+="       iflags_pixel_bad=False and\n"
    #stout+="       iflags_pixel_suspect_center=False"
# and\n"
...
field_type = ["udeep","deep","wide"]
field_name = ["sxds","deep2_3","vvds"]
for typ,fld in zip(field_type,field_name):
    write_fieldsearch("pdr1_"+typ,fld,"field_"+typ+"_"+fld+".sql",do_field=True,submit=submit)

exit(1)

#Per-frame metadata
print 'Writing frames sql'
write_frames("pdr1_wide","frames_wide.sql",submit=submit)
write_frames("pdr1_deep","frames_deep.sql",submit=submit)
write_frames("pdr1_udeep","frames_udeep.sql",submit=submit)

#WIDE fields
print 'Writing WIDE fields sql'
for fld in ['gama09h','gama15h','hectomap','wide12h','xmm_lss','aegis'] :
    write_fieldsearch("pdr1_wide",fld,"field_wide_"+fld+"_pz.sql",do_field=True,submit=submit,do_photoz=True)
write_fieldsearch("pdr1_wide",'vvds',"field_wide_vvds_h1.sql",do_field=True,submit=submit,ra_range=[330.,336.],do_photoz=True)
write_fieldsearch("pdr1_wide",'vvds',"field_wide_vvds_h2.sql",do_field=True,submit=submit,ra_range=[336.,342.],do_photoz=True)

#DEEP fields
print 'Writing DEEP fields sql'
for fld in ['cosmos','deep2_3','elais_n1','xmm_lss'] :
    write_fieldsearch("pdr1_deep",fld,"field_deep_"+fld+".sql",do_field=True,submit=submit,do_photoz=True)

#UDEEP fields
print 'Writing UDEEP fields sql'
for fld in ['cosmos','sxds'] :
    write_fieldsearch("pdr1_udeep",fld,"field_udeep_"+fld+".sql",do_field=True,submit=submit,do_photoz=True)

#WIDE-depth COSMOS
print 'Writing WIDE depth COSMOS sql'
for see in ['best','median','worst'] :
    write_fieldsearch("pdr1_cosmos_widedepth_"+see,"none","field_cosmo_wide_"+see+".sql",do_field=False,submit=submit,do_photoz=False)

#WIDE field randoms
print 'Writing WIDE field randoms sql'
for fld in ['gama09h','gama15h','hectomap','wide12h','xmm_lss','aegis'] :
    write_fieldsearch("pdr1_wide",fld,"field_wide_"+fld+"_randoms.sql",do_field=True,submit=submit,do_photoz=False,is_random=True)
write_fieldsearch("pdr1_wide",'vvds',"field_wide_vvds_h1_randoms.sql",do_field=True,submit=submit,ra_range=[330.,336.],do_photoz=False,is_random=True)
write_fieldsearch("pdr1_wide",'vvds',"field_wide_vvds_h2_randoms.sql",do_field=True,submit=submit,ra_range=[336.,342.],do_photoz=False,is_random=True)
...
for fld in ['aegis','gama09h','gama15h','hectomap','wide12h','xmm_lss'] :
    write_fieldsearch("pdr1_wide",fld,"field_wide_"+fld+".sql",do_field=True,submit=submit)
#VVDS is too large for a single query
write_fieldsearch("pdr1_wide",'vvds',"field_wide_vvds_h1.sql",do_field=True,submit=submit,ra_range=[330.,336.])
write_fieldsearch("pdr1_wide",'vvds',"field_wide_vvds_h2.sql",do_field=True,submit=submit,ra_range=[336.,342.])

#WIDE-depth COSMOS
for see in ['best','median','worst'] :
    write_fieldsearch("pdr1_cosmos_widedepth_"+see,"none","field_cosmo_wide_"+see+".sql",do_field=False,submit=submit)

#DEEP fields
for fld in ['cosmos','deep2_3','elais_n1','xmm_lss'] :
    write_fieldsearch("pdr1_deep",fld,"field_deep_"+fld+".sql",do_field=True,submit=submit)

#UDEEP fields
for fld in ['cosmos','sxds'] :
    write_fieldsearch("pdr1_udeep",fld,"field_udeep_"+fld+".sql",do_field=True,submit=submit)
