...
def main():
    ...
    parser.add_argument('--api-url', default='https://hsc-release.mtk.nao.ac.jp/datasearch/api/catalog_jobs/',
                        help='for developers')
    parser.add_argument('--download','-d', default=False, action='store_true', help='activate download')
...
#    credential = {'account_name': args.user, 'password': getPassword()}
    ...
    try:
        if args.preview:
            ...
            print 'Job submitted: ',sql
            if args.download:
                blockUntilJobFinishes(credential, job['id'])
                print 'Downloading job',str(job['id'])
                download(credential, job['id'], out = open('./'+str(job['id'])+'.'+args.out_format,"w"))
                print 'Done!'
#            if args.delete_job:
